package com.astamuse.asta4d.ide.eclipse.contenttype;
...
public class ContentDescriberForAsta4d implements ITextContentDescriber {
    ...
    private int checkCriteria(Reader contents) throws IOException {
        final Pattern nsNamePattern = Pattern.compile("afd:.*");
        ...
        while (token != null && !DOMRegionContext.XML_TAG_CLOSE.equals(token) && !DOMRegionContext.XML_EMPTY_TAG_CLOSE.equals(token)) {
            ...
            if (DOMRegionContext.XML_TAG_NAME.equals(token) || DOMRegionContext.XML_TAG_ATTRIBUTE_NAME.equals(token)) {
                checkName = tokenizer.yytext();
            }
            if (nsNamePattern.matcher(checkName).matches()) {
                return VALID;
            }
